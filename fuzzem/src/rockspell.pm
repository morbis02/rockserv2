...
sub spell_astral_spy {
    ...
    if($victim->{'HIDDEN'} == 1) {
        $self->log_append("{17}The spy can not see $victim->{'NAME'}.\n");
        return 0;
    }
...
sub spell_wizards_eye {
    ...
    my ($self, $vname) = @_;
     my $racemono= 0;
 	if( $self->race_owns_monolith('monolith_optical') && $self->{'TYPE'}==1  && $self->{'SOLDIER'}){
	$racemono = 1;
    }
    if(!$self->skill_has(39) && ! $racemono ) {
    ...
    if($victim->{'HIDDEN'}){$self->log_error("$victim->{'NAME'} is obfuscated and can not be seen."); return 0; }
...
sub spell_blinding {
    ...
    if(rand(1) < $self->fuzz_pct_skill(22,100) && !$victim->{'NOSICK'} ) {
...
sub spell_critical_heal {
    my ($self) = @_;
    if(!$self->skill_has(80)) {
        $self->log_error('You have no idea how to heal critical wounds.');
        return 0;
    }
    
        return 0 if(!$self->can_do(30, 0, 25));

    $self->log_append("{12}Some of your critical wounds are miraculously healed.\n");
    $self->room_sighttell("{12}$self->{'NAME'}\'s critical wounds vanish miraculously.\n");
    $self->{'HP'} += 2* (int ($self->fuzz_pct_skill(11)*$self->{'MAXH'} / (5 + 15 * $self->is_tired()))  );
...
sub spell_close_wounds {
    my ($self) = @_;
    my $racemono= 0;
 	if( $self->race_owns_monolith('monolith_protector') && $self->{'TYPE'}==1  && $self->{'SOLDIER'}){
	 $racemono = 1;
     }
    if(!$self->skill_has(4) && !$racemono ) {
...
sub spell_knit_flesh {
    my ($self) = @_;
    my $racemono= 0;
 if( $self->race_owns_monolith('monolith_protector') && $self->{'TYPE'}==1 ){
	 $racemono = 1;
     }
    if(!$self->skill_has(19) && !$racemono ) {
        $self->log_error('You have no idea how your body could close its wounds.');
        return 0;
    }

    return 0 if(!$self->can_do(5, 0, 2));
...
sub spell_speed_healing {
    my ($self) = @_;
    my $racemono= 0;
 	if( $self->race_owns_monolith('monolith_protector') && $self->{'TYPE'}==1  && $self->{'GENERAL'}){
	 $racemono = 1;
     }
    if(!$self->skill_has(5) && !$racemono ) {
...
sub spell_generic (cap) {
    ...
    if($vname) {
        my ($success, $item) = $self->inv_cgetobj($vname, 0, $main::map->[$self->{'ROOM'}]->inv_pobjs, $self->inv_pobjs);
        if($success == 1) {
...
sub spell_hgeneric (spell_code[, receiver object]) {
	...
	#	$self->log_error($main::spellbase{uc($spellcode)});
...
if(($all == 1) && ($rectype == -1)){
	my $mintemp;
	foreach my $victimtemp (@victims) {
		$mintemp = $mintemp + $mint;
	}
	$mint = $mintemp;
	}
...
$acode_is_array = 1;
    ...
    foreach $victim (@victims) {
        ...
        $self->damAttack($victim, $dam) if($dam); #DEALS X DAMAGE
...
#    }
    ...
    if(!$self->skill_has(82) && !$self->inv_rec_scan(933) ){    
    	if(($totalinv > $maxinventory) && (rand(100)<3) ){ 
   		 my $item = $self->inv_rand_item_unequipped();
    	$self->log_append("$item->{'NAME'} falls to the ground due to lack of space in your inventory.\n");
    	$self->item_drop($item->{'NAME'});
		}
	}
...
$self->{'MA'} = $self->{'MAXM'} if $self->{'TYPE'} != 1;
    ...
    if( $room->{'ACTFAIL'} && (rand(100) < $room->{'ACTFAIL'}) ) {
        if( $room->{'ACTFAIL'} == 33 && $self->aprl_rec_scan(940))
        { 
    ...
    if(  (defined($self->{'FX'}->{71}) && !$self->room()->{'SAFE'} ) ) { 
        my $dam = int rand($self->{'MAXH'} / 15);
        if($dam && !$self->{'NEWBIE'} && !$self->is_dead) {
            $self->log_append("{1}You are doubled up in pain as the deadly poison eats away at your organs, causing {17}$dam {1}damage!\n");
            $self->room_sighttell("{17}$self->{'NAME'} is doubled up in pain as the poison eats away at their organs, inflicting {17}$dam {1}damage!\n");
            $self->{'HP'} -= $dam;
            if($self->is_dead) {
                $self->die($self);
                return 0;
...
sub skill_decapitate {
	...
	if($self->{'DECAP_USED'} >= 1){$self->log_append("You have used your decapitation for the day\n"); return 0;}
    ...
    if($victim->{'TYPE'} >= 1)
    {
	    unless ($self->log_cant_aggress_against($victim, 1)) {
		    if(rand(10) < 6){ $self->hdecapitate($victim);}
		    else{$self->log_append("You fail to decapitate $victim->{'NAME'}\n");}
		}
    }
...
sub hdecapitate {
    ...
    if ($victim->{'TYPE'} == 1 && !$victim->inv_rec_scan(1060) ) {
        $victim->{'HP'}=-1;

	    $victim->die($self);
        $self->{'DECAP_USED'} ++;
    } else {
        $victim->room_sighttell("{14}$victim->{'NAME'} {4}growa new head.\n");
        $self->{'DECAP_USED'} ++;
    }
}
...
sub spell_tornado {
    ...
    if($main::map->[$victim->{'ROOM'}]->{'EXITS'}) { 
        $victim->room_sighttell("{4}A tornado picks {16}$victim->{'NAME'} {4}up.\n");
        $victim->log_append("{4}A tornado picks you up.\n");
        $victim->ai_move(&rockobj::ai_suggest_move_random(undef, $main::map->[$victim->{'ROOM'}]->exits_hash));
    }
...
sub spell_stone_skin {
    # NOTE, the $self is the object that gets melded, NOT the user melding.
    my ($self) = @_;
    
    if(!$self->skill_has(76)) {
        $self->log_error('Calm down, you have no idea to cast stone skin.');
        return 0;
    }
    ...
    $self->effect_add(82);
    $self->room_sighttell("{3}$self->{'NAME'}\'s skin is suddenly hard like concrete $self->{'PPOS'} solidifies.\n");
    return 1;
}

sub spell_deafen {
    my ($self, $victim) = @_;
    # if i dont have the deafen skill, tell me and exit
    if(!$self->skill_has(77)) {
        $self->log_error('You have no idea how to deafen others.');
        return 0;
    }
    
    # if pvp restrictions dont support it, or i dont have the turns/mana, tell me and exit
    return 0 if($self->log_cant_aggress_against($victim) || !$self->can_do(40, 0, 15));
    
	# Log attack for pvp info
	$self->note_attack_against($victim); 
	
    # Tell myself and the room what happened.
    $self->log_append("{5}You cast the spell of {15}deafen{5} upon {2}$victim->{'NAME'}\{5}.\n");
    $self->room_sighttell("{1}$self->{'NAME'} {5}casts the spell of {15}deafen{5} upon {2}$victim->{'NAME'}\{5}.\n");
    
    # if (random number, compared to stat 22) works out, then deafen the victim.
    if(rand(1) < $self->fuzz_pct_skill(22,100)) {
        $victim->effect_add(25);
        $victim->{'FX'}->{'25'} = time + 90;
        $victim->effects_update();
    } else {
        $victim->log_append("{1}You are unaffected.\n");
    }
...
sub spell_leach {
    my ($self, $victim) = @_;
    # if i dont have the silence skill, tell me and exit
    if(!$self->skill_has(78)) {
        $self->log_error('You have no idea how to leach others.');
        return 0;
    }
    
    # if pvp restrictions dont support it, or i dont have the turns/mana, tell me and exit
    return 0 if($self->log_cant_aggress_against($victim) || !$self->can_do(75, 0, 50));
    
	# Log attack for pvp info
	$self->note_attack_against($victim); 
	
    # Tell myself and the room what happened.
    $self->log_append("{5}You cast the spell of {15}leach{5} upon {2}$victim->{'NAME'}\{5}.\n");
    $self->room_sighttell("{1}$self->{'NAME'} {5}casts the spell of {15}leach{5} upon {2}$victim->{'NAME'}\{5}.\n");
    
    # if (random number, compared to stat 22) works out, then silence the victim.
    if(rand(1) < $self->fuzz_pct_skill(22,100)) {
        $victim->effect_add(83);
        $victim->{'FX'}->{'83'} = time + 30;
        $victim->effects_update();
    } else {
        $victim->log_append("{1}You are unaffected.\n");
    }
...
sub spell_berserk {
    ...
    #es your MAXH by an amount equal to your MDEF + manaused + LEV
    if(!$override) {
        if(!$self->skill_has(90)) {
            $self->log_error('Yeah right - you dont know how to go crazy in such  a way!');
            return 0;
        }
        
		if($self->{'FX'}->{'93'}) {
        $self->log_error('You are unable to become further enraged at this time.');
        return 0;
		}
		
		return 0 if(!$self->can_do(0, int ($self->{'MAXH'}/5), 36));
    }
    ...
    $self->room_sighttell("{4}$self->{'NAME'}\'s turn bright red, as $self->{'PRO'} becomes enraged.\n");
    $self->log_append("{4}Your eyes turn bright red as you go berserk.\n");

...
sub can_enter() {
 ...
 if(!$_[1]->inv_has_rec(337) && !$_[1]->inv_has_rec(363)) { 
    $_[1]->log_append("{1}Sorry, but you need a plastic ruler to enter $_[0]->{'NAME'}.\n"); return(0); }
 return(1);
}
...
sub min_pool_room::can_enter($playerobject) {
 ...
 if($_[0]->{'TER'}==18 && !$_[1]->{'FX'}->{'7'} && !$_[1]->skill_has(36)) { $_[1]->log_append("{12}## Go there? Do you want to drown or something?! ##\n"); return(0); }
...
sub cleanup_inactive {
 ...
 # ONLY IF ITS BASED ON ROOM, NOT CBY $self->room_sighttell('{6}Cleaning Nullity: {16}'.join(', ')."\n");
  if($self->{'NULLIFIED'}>15) { &main::spawn_stuff; delete $self->{'NULLIFIED'}; }
  delete $self->{'CRYL'};
  return;
}
